...
export const apiModifyPosting = async (postingID, updateObject) => {
    ...
    try {
        const res = await axios.patch(`${API_HOST}/api/admin/postings/${postingID}`, updateObject); 
        return res.data; 
    } catch(error) {
        if(error.response.status == 401) {
            logoutHelper(); 
...
export const apiGetApplicationsForPosting = async (postingID) => {
    ...
    try {
        const res = await axios.get(`${API_HOST}/api/admin/applications/${postingID}`); 
        return res.data; 
    } catch(error) {
        if(error.response.status == 401) {
            logoutHelper(); 
...
export const apiApproveApplication = async (applicationID) => {
    ...
    try {
        const res = await axios.get(`${API_HOST}/api/admin/applications/approve/${applicationID}`); 
        return res.data; 
    } catch(error) {
        if(error.response.status == 401) {
            logoutHelper(); 
...
export const apiApproveApplication = async (applicationID) => {
    ...
    try {
        const res = await axios.get(`${API_HOST}/api/admin/applications/reject/${applicationID}`); 
        return res.data; 
    } catch(error) {
        if(error.response.status == 401) {
            logoutHelper(); 
