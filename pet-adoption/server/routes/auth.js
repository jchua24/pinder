...
router.post("/login", mongoChecker, sessionChecker, async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .send("Invalid request - email or password is missing.");
  }
  ...
  try {
    ...
    return res
      .status(404)
      .send("User not found with specified email/password.");
  } catch (error) {
    console.log(error);
...
router.post("/add", sessionChecker, async (req, res) => {
  ...
  if (
    ...
    !req.body.hasOwnProperty("province") ||
    !req.body.hasOwnProperty("postal") ||
    !req.body.hasOwnProperty("phone") ||
    !req.body.hasOwnProperty("admin")
  ) {
    console.log("Missing a param of the user");
    return res
      .status(400)
      .send("Invalid request - email, password or name is missing.");
  }
